            ...
            for andSplit in split_and:
                ...
                for word in andsplit_space:
                    if word.lower() in ['comp', 'math', 'stat']:
                        ...
                        coursenumber = andsplit_space[index+1].strip().split("(")
                        name = word+" "+coursenumber[0].strip(';.,')
                        id = word+coursenumber[0].strip(';.,')
                        if name in dictYear[0].values():
                            prerequisiteList.append(id)
                            dict2[fields[7]] = prerequisiteList
                            break
                        if name in dictYear[1].values():
                            prerequisiteList.append(id)
                            dict2[fields[7]] = prerequisiteList
                            break
                        if name in dictYear[2].values():
                            prerequisiteList.append(id)
                            dict2[fields[7]] = prerequisiteList
                            break
                        if name in dictYear[3].values():
                            prerequisiteList.append(id)
                            dict2[fields[7]] = prerequisiteList
                            break
                        if name in dictProg[0].values():
                            prerequisiteList.append(id)
                            dict2[fields[7]] = prerequisiteList
                            break
                        if name in dictProg[1].values():
                            prerequisiteList.append(id)
                            dict2[fields[7]] = prerequisiteList
                            break
        else:
